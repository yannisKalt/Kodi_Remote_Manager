...
def resolve(url):
		...
		if 'tvcatchup' in url:
			...
			headers1 = {'Authorization': 'Basic QFN3aWZ0MTUjOkBTd2lmdDE1Iw==',
				'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 7.1.2; ONE A2003 Build/NJH47B)'}

			headers2 = {'Authorization': 'Basic QFN3aWZ0MTEjOkBTd2lmdDExIw==',
				'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 7.1.2; ONE A2003 Build/NJH47B)'}
			...
			if 'fixed' in url:
				...
				ope = requests.session().get('http://173.212.202.101/token15001.php',headers=headers2).text
				open=ope.strip('eMeeea/1.0.0.')
			else:
				open = requests.session().get('http://173.212.202.101/token1004.php',headers=headers).text
