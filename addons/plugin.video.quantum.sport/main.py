...
if mode is None:
    ...
    addDir('QUANTUM LIVE',ex_link='',params={'_service':'sport365','_act':'ListChannels'}, mode='site2',iconImage=RESOURCES+'sport365.png')
    ...
    if error_msg:
        ...
        if len(out_sum)>1:
            xbmcgui.Dialog().ok('[COLOR green]Lijst opgeslagen[/COLOR] Kanałów: %d'%len(out_sum),'Plik: [COLOR blue]'+fname+'[/COLOR]','Uaktualnij ustawienia [COLOR blue]PVR IPTV Simple Client[/COLOR] i (re)aktywuj Live TV')
        else:
            xbmcgui.Dialog().ok('[COLOR red]Probleem[/COLOR] ','Lista jest Pusta!!!')     
    my_addon.openSettings() 
    ...
    if act == 'get_streams_play':
        ...
        if streams:
            ...
            s = xbmcgui.Dialog().select("Selecteer uw bron", t)
            ...
            xbmcgui.Dialog().ok("Probleem", 'Geen bronnen beschikbaar')  
