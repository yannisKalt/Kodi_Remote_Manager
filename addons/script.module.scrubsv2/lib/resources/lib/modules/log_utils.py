    ...
    try:
        ...
        if not control.setting('debug.location') == '0':
            ...
            if not os.path.exists(log_file):
                f = open(log_file, 'w'); f.close()
            with open(log_file, 'a') as f:
                debug_enabled = control.setting('addon_debugtime')
                if not control.setting('addon_debugtime') == 'true':
                    line = '%s: %s' % (DEBUGPREFIX, msg)
                else:
                    line = '[%s %s] %s: %s' % (datetime.now().date(), str(datetime.now().time())[:8], DEBUGPREFIX, msg)
                if not control.setting('addon_debugspacer') == 'true':
                    f.write(line.rstrip('\r\n') + '\n')
                else:
                    f.write(spacer() + '\n')
                    f.write(line.rstrip('\r\n') + '\n')
