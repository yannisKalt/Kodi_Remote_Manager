...
# Doko'S Dupe Checker for cleaning results.
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, quality='HD', timeout=30):
        ...
        for i in range(0, 4 * timeout):
            ...
            try:
                ...
                if debrid_status:
                    d_4k_label = total_format % ('darkorange', d_source_4k) if d_source_4k == 0 else total_format % ('lime', d_source_4k)
                    d_1080_label = total_format % ('darkorange', d_source_1080) if d_source_1080 == 0 else total_format % ('lime', d_source_1080)
                    d_720_label = total_format % ('darkorange', d_source_720) if d_source_720 == 0 else total_format % ('lime', d_source_720)
                    d_sd_label = total_format % ('darkorange', d_source_sd) if d_source_sd == 0 else total_format % ('lime', d_source_sd)
                    d_total_label = total_format % ('darkorange', d_total) if d_total == 0 else total_format % ('lime', d_total)
    ...
    def sourcesFilter(self):
        ...
        sRapGat = control.setting('showRapGat')
...
###
            ...
            if d:
                if 'torrent' in s.lower():
                    if not torr_identify == 'nocolor':
                        self.sources[i]['label'] = ('[COLOR %s]' % (torr_identify)) + label + '[/COLOR]'
                    else:
                        self.sources[i]['label'] = label # label.upper()
                else:
                    if not prem_identify == 'nocolor':
                        self.sources[i]['label'] = ('[COLOR %s]' % (prem_identify)) + label + '[/COLOR]'
                    else:
        ...
        try: 
            if not sRapGat == 'true':
                self.sources = [i for i in self.sources if not 'rapidgator' in i['url']]
        except:
            pass
