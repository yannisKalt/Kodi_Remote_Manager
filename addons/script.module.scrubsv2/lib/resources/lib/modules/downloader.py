...
def doDownload(url, dest, title, image, headers):
    ...
    if not resp:
        xbmcgui.Dialog().ok(title, dest, 'Sorry, download failed!', 'No response from the server.')
        return
    try:
    ...
    resume  = 0
    sleep   = 0
    if xbmcgui.Dialog().yesno(title + ' - Confirm Download', file, 'Complete file is: %dMB' % mb, 'Continue with download?', 'Yes',  'No') == 1:
        return
