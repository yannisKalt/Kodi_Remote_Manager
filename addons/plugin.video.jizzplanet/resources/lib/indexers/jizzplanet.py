...
class indexer:
    ...
    def root(self):
        try:
            regex.clear()
            url = 'http://jizzplanet.com/repo/xmls/xxxmain.xml'
            self.list = self.jizzplanet_list(url)
    ...
    def developer(self):
        try:
            ...
            self.list = self.jizzplanet_list('', result=result)
    ...
    def addSearch(self, url=None):
        try:
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.jizzplanet_list, link))
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['jizzplanet TV']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
